...
export class SightFormComponent implements OnInit {
	...
	constructor(private sightService: SightService,private uploadService: UploadFilesService, private discountService: DiscountService,private tagService: TagService,
		private formBuilder: FormBuilder, private countryService: CountryService, private cityService: CityService, private router: Router, private route: ActivatedRoute) {
	...
	onFileSelect(event) {
	  ...
	  onFormSubmit() {
		...
		this.sightService.UploadImage(formData).subscribe(response => {
