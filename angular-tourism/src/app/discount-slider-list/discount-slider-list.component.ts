...
@Component({
  selector: 'app-discount-slider-list',
  templateUrl: './discount-slider-list.component.html',
  styleUrls: ['./discount-slider-list.component.css']
})
export class DiscountSliderListComponent implements OnInit {
	total:any;
	page:any;
	pageSize:any;
	list_sights: any;
	...
	constructor(private arService: DiscountSliderService, private sightService: DiscountService, private router: Router) { }
	...
	retrieveAr(): void {
		...
		this.arService.getAll()
          ...
          for (var i = 0; i < data.length; i++) {
						if(data[i].discount_id != undefined && this.list_sights.find(s=>s.id == data[i].discount_id) != undefined){
							data[i].discount_id = this.list_sights.find(s=>s.id == data[i].discount_id).main_header_en;
