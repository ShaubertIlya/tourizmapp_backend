...
export class ArFormComponent implements OnInit {
	...
	constructor(private arService: ArService,
		...
		private sightService: SightService, private router: Router, private route: ActivatedRoute) {
	...
	onFileSelect(event) {
	  ...
	  onFormSubmit() {
		...
		formData.append('uploadedFile', this.fileUploadForm.get('uploadedImage').value);
		formData.append('agentId', '007');
	
		this.arService.UploadImage(formData).subscribe(response => {
			...
			if (response.statusCode === 200) {
			  ...
			  this.file = "http://185.113.134.76:8080/my_files/"+response.uploadedFile.filename
	...
	onFileSelect2(event) {
	  ...
	  onFormSubmit2() {
		...
		const formData = new FormData();
		formData.append('uploadedFile', this.fileUploadForm2.get('uploadedImage2').value);
		...
		this.arService.UploadImage(formData).subscribe(response => {
			...
			if (response.statusCode === 200) {
			  ...
			  this.file2 = "http://185.113.134.76:8080/my_files/"+response.uploadedFile.filename
